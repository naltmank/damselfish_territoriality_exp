...
(territory_plot <- ggplot(data = change, aes(x = Fertilized, y = Surface.area.pct.change) )+
  ...
  theme_classic() + 
  labs(x ="Fertilized (Y/N)", y = "Percent change in area", title = "(a)\n") +
...
(fish_plot <- ggplot(data = change, aes(x = Fertilized, y = Fish.pct.change) )+
  ...
  scale_x_discrete(limits = c('N', 'Y'), name = "Treatment",
                   labels = c("N" = "Unfertilized", "Y" = "Fertilized")) + 
  theme_classic() + 
  labs(x ="Fertilized (Y/N)", y = "Percent change in resident fish", title = "(b) \n") +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        legend.position = "none",
   ...
   geom_hline(yintercept=0, linetype = "dashed")  +
  annotate("text", x = 0.6, y = 185, label = "LMER\nChi-sq = 0.14, P = 0.71", hjust = 0, size = 10) 
  )
```
## create panel graph
```{r territory characteristics panel, fig.height=10, fig.width=18}
(territory_traits <- ggpubr::ggarrange(territory_plot, fish_plot, nrow = 1, widths = c(1,1)))

ggsave(here("output", "fig1.eps"), territory_traits, width = 18, height = 10, units = "in", dpi = "retina")
```
...
# check that double offset is equivalent to combined offset
bite_mod_1a <- glmmTMB(Bites ~ Pre.post.trt*Fertilized + (1|Pair/Territory) + offset(log(Time.guarding.min)) + offset(log(Surface.area)), family = poisson, data = behavior )
summary(bite_mod_1a) # it is
...
behavior_means$Bite.rate.letter.height <- (behavior_means$Bite.rate.per.m2 + behavior_means$Bite.rate.per.m2_se)*1.15 
...
(bite_rate_graph <- ggplot()+
  ...
  geom_jitter(data = behavior, aes(x = Pre.post.trt, y = Bite.rate.per.m2, colour = Fertilized),
              alpha = 1, shape = 4, size = 4, width = 0.1) +
...
#            hjust = 0, size = 8) + 
  ...
  scale_colour_manual(name = "Treatment", labels = c("Unfertilized", "Fertilized"),
                        values = c("N" = "darkred", "Y" = "dodgerblue")) +
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        panel.background = element_blank(),
        axis.line = element_line(colour = "black", size = 1.2),
        plot.title = element_text(color = "black", size = 32, vjust = 0, face = "plain"),
        axis.line.y = element_line(colour = "black"),
        axis.text.y = element_text(color = "black", size = 28, hjust = .5, vjust = .5, face = "plain"),
        axis.title.y = element_text(color = "black", size = 32, hjust = .5, vjust = 0, face = "plain",
                                   margin = margin(r = 15)),
        axis.line.x = element_line(colour = "black"),
#        axis.text.x = element_text(color = "black", size = 28, hjust = .5, vjust = .5, face = "plain"),
#        axis.title.x = element_text(color = "black", size = 32, hjust = .5, vjust = 0, face = "plain"),
    ...
    annotate("text", x = 2.4, y = 0.55, label = "**", vjust = 0.5, size = 12))
...
otherbites_means$Letter.height <-
  (otherbites_means$Other.fish.bites.rate.per.m2 +
   otherbites_means$Other.fish.bites.rate.per.m2_se) * 1.15
```
...
(otherbite_rate_graph <- ggplot()+
  ...
  geom_errorbar(data = otherbites_means, aes(x = Pre.post.trt, ymin = Other.fish.bites.rate.per.m2 - Other.fish.bites.rate.per.m2_se,
             ...
             size = 0.6, position = position_dodge(width = 0.2), width = 0.15) +
  geom_jitter(data = otherbites, aes(x = Pre.post.trt, y = Other.fish.bites.rate.per.m2,
                                     colour = Fertilized),
              alpha = 1, shape = 4, size = 4, width = 0.1) +
...
#  scale_y_continuous(limits = c(0, 0.006)) +
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        axis.line.y = element_line(colour = "black"),
        axis.text.y = element_text(color = "black", size = 28, hjust = .5, vjust = .5, face = "plain"),
        axis.title.y = element_text(color = "black", size = 32, hjust = .5, vjust = 0, face = "plain",                                    margin = margin(r = 15)),
        axis.line.x = element_line(colour = "black"),
#        axis.text.x = element_text(color = "black", size = 28, hjust = .5, vjust = .5, face = "plain"),
#        axis.title.x = element_text(color = "black", size = 32, hjust = .5, vjust = 0, face = "plain"),
        axis.text.x =  element_blank(),
...
behavior_means$Intra.chase.letter.height <- (behavior_means$Intra.chase.rate.per.m2 + behavior_means$Intra.chase.rate.per.m2_se)*1.2

```

```{r intraspecific chases graph, fig.height=10, fig.width=10}
(intra_chase_rate_graph <- ggplot()+
  ...
  geom_errorbar(data = behavior_means, aes(x = Pre.post.trt, ymin = Intra.chase.rate.per.m2 - Intra.chase.rate.per.m2_se,
             ...
             size = 0.6, position = position_dodge(width = 0.2), width = 0.15) +
  geom_jitter(data = behavior, aes(x = Pre.post.trt, y = Intra.chase.rate.per.m2, colour = Fertilized), alpha = 1, shape = 4,
            size = 4, width = 0.1) +
...
#             hjust = 0, size = 8) + 
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        legend.position = 'bottom', # move legend to bottom
        legend.box = 'horizontal',
        legend.box.background = element_rect(colour = NA), # make legend horizontal
        legend.key=element_blank()
   ))
...
(inter_chase_rate_graph <- ggplot()+
  ...
  geom_errorbar(data = behavior_means, aes(x = Pre.post.trt, ymin = Inter.chase.rate.per.m2 - Inter.chase.rate.per.m2_se,
             ...
             size = 0.6, position = position_dodge(width = 0.2), width = 0.15) +
  geom_jitter(data = behavior, aes(x = Pre.post.trt, y = Inter.chase.rate.per.m2, colour = Fertilized), alpha = 1, shape = 4,
           size = 4, width = 0.1) +
...
(behavior_panel <- ggpubr::ggarrange(bite_rate_graph, otherbite_rate_graph, intra_chase_rate_graph, inter_chase_rate_graph, 
...
ggsave(here("output", "fig2.eps"), behavior_panel, width = 18, height = 15, units = "in", dpi = "retina")
```
...
#### CONDENSING TRIPLICATES - SAME RESULTS AND ISSUES ####
bio_graph_2 <- ddply(bio, c("Fertilized", "Pre.post.trt", "Territory", "Pair"), numcolwise(mean), na.rm = TRUE ) 
biomass_mod_2 <- glmmTMB(Biomass ~ Pre.post.trt*Fertilized + offset(log(Area)) + (1|Pair) , family = Gamma(link = "log"), data = bio_graph_2)
Anova(biomass_mod_2)
plot(simulateResiduals(biomass_mod_2))
emmeans::emmeans(biomass_mod_2, pairwise ~ Pre.post.trt | Fertilized)
emmeans::emmeans(biomass_mod_2, pairwise ~ Fertilized | Pre.post.trt)
...
bio_graph$Letter_height <- (bio_graph$Biomass.per.cm2 + bio_graph$Biomass.per.cm2.SE)*1.15
...
(bio_plot <- ggplot() + 
  ...
  geom_point(data = bio_graph, aes(x= Pre.post.trt, y = Biomass.per.cm2, fill = Fertilized, 
             colour = Fertilized), size = 8, position = position_dodge(width = 0.2)) + # add points for mean
  ...
  geom_text(data = bio_graph, aes(x = Pre.post.trt, y = Letter_height, colour = Fertilized, label = Letters),
            size = 8, position = position_dodge(width = 0.2), show.legend = FALSE) +
  geom_jitter(data = bio, aes(x = Pre.post.trt, y = Biomass.per.cm2, colour = Fertilized),
              alpha = 1, shape = 4, size = 4, width = 0.1) +
  ...
  theme(panel.grid.major = element_blank(), 
    ...
    annotate("text", x = 0.5, y = 0.3, label = "Timepoint: P <0.001\nTreatment: P = 0.048\nTimepoint x Treatment: P = 0.16",
             hjust = 0, size = 8) +
    ...
    annotate("text", x = 2.3, y = 0.0234, label = "*", vjust = 0.5, size = 12))
...
(turf_traits <- ggpubr::ggarrange(bio_plot, cn_plot, nrow = 1, widths = c(1,1)))
ggsave(here("output", "fig3.eps"), turf_traits, width = 18, height = 10, units = "in", dpi = "retina")
```
